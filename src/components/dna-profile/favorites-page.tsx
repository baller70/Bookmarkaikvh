...
// Replace MockBookmark with API-mapped display type
...
export function FavoritesPage({ userId }: FavoritesPageProps) {
  ...
  const [filteredBookmarks, setFilteredBookmarks] = useState<DisplayBookmark[]>([]);
  ...
  const mapApiToDisplay = useCallback((api: ApiBookmark): DisplayBookmark => ({
    id: api.id,
    title: api.title,
    url: api.url,
    description: api.description,
    is_favorite: api.isFavorite === true,
    visit_count: typeof api.visits === 'number' ? api.visits : 0,
    created_at: api.created_at,
    updated_at: api.updated_at,
    folder: api.category ? { name: api.category } : undefined,
    tags: Array.isArray(api.tags) ? api.tags.map(name => ({ name })) : undefined,
    __raw: api
  }), [])
  ...
  const loadData = useCallback(async (isRefresh = false) => {
    try {
      ...
      const apiBookmarks: ApiBookmark[] = Array.isArray(json.bookmarks) ? json.bookmarks : []
      const favorites = apiBookmarks.filter(b => b.isFavorite === true)
      setBookmarks(favorites.map(mapApiToDisplay))
    } catch (e) {
  ...
  const applyFiltersAndSort = useCallback(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(b => (
        ...
        b.tags?.some(tag => tag.name.toLowerCase().includes(term))
      ))
    }
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'created_at': aValue = new Date(a.created_at ?? 0); bValue = new Date(b.created_at ?? 0); break
        case 'updated_at': aValue = new Date(a.updated_at ?? 0); bValue = new Date(b.updated_at ?? 0); break
        case 'visit_count': aValue = a.visit_count || 0; bValue = b.visit_count || 0; break
        case 'folder': aValue = a.folder?.name || ''; bValue = b.folder?.name || ''; break
        default: return 0
      }
  ...
  const handleToggleFavorite = useCallback(async (bookmark: DisplayBookmark) => {
    ...
    try {
      ...
      if (result?.success) {
        setBookmarks(prev => prev.map(b => b.id === bookmark.id ? { ...b, is_favorite: newStatus, __raw: { ...raw, isFavorite: newStatus } } : b))
        setFilteredBookmarks(prev => prev.map(b => b.id === bookmark.id ? { ...b, is_favorite: newStatus, __raw: { ...raw, isFavorite: newStatus } } : b))
        if (!newStatus) {
          setBookmarks(prev => prev.filter(b => b.id !== bookmark.id))
          setFilteredBookmarks(prev => prev.filter(b => b.id !== bookmark.id))
        }
      } else {
          ...
          <Card key={bookmark.id} className="hover:shadow-lg transition-shadow">
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-3">
                ...
                >
                  <Heart className={`h-5 w-5 ${bookmark.is_favorite ? 'text-red-500 fill-current' : 'text-gray-400'}`} />
                </button>
              </div>
