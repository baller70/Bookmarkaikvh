  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.success && result.url) {
        // Update the avatar URL (UI state)
        setEditedProfile({
          ...editedProfile,
          avatar: result.url
        })

        // Persist globally so Dashboard avatars use this as default
        try {
          // Primary key used by dashboard utils
          localStorage.setItem('profilePicture', result.url)

          // Also persist in userSettings.profile for broader compatibility
          ...
          console.warn('Failed to persist global avatar settings:', err)
        }
