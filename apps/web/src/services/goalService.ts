...
class GoalService {
  private async getCurrentUserId(): Promise<string | null> {
    try {
      ...
      console.warn('Supabase auth not available, using fallback user ID')
    }
  ...
  async getGoalFolders(): Promise<GoalFolder[]> {
    try {
      ...
      if (!userId) {
        console.warn('No user ID available, returning empty goal folders')
        return []
      }
      ...
      console.log('🎯 getGoalFolders: Response data =', data)
      ...
      console.log('✅ getGoalFolders: Success, returning', data.data?.length || 0, 'folders')
      return data.data || []
    } catch (error) {
