    ...
    <div className="space-y-6 p-4">
              ...
              <CardContent className="space-y-6">
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between">
                            <div className="text-xs text-gray-500 truncate flex-1 mr-2">
                              {asset.url && new URL(asset.url).hostname}
                            </div>
                ...
                <TaskManagementSection
                  data={{
                    ...
                    reminderEnabled: section.reminderEnabled,
                    reminderDate: section.reminderDate
                  }}
                  onChange={(updates) => updateTaskManagement(section.id, updates)}
                ...
                <CommentsSection
                  comments={section.comments}
                  onAddComment={(content) => handleAddComment(section.id, content)}
                  sectionTitle={section.title || `Section ${index + 1}`}
                />
      ...
      {currentSectionId && (
        <AddAssetsModal
          ...
          availableAssets={filteredFiles.map(file => ({
            id: file.id,
            name: file.name,
            type: getAssetType(file.type || file.mime_type),
            url: file.url,
            size: file.size,
            uploadedAt: new Date(file.uploadedAt || file.created_at)
          }))}
