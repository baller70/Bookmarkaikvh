...
function PoolKanbanCard({ bookmark, index }: { bookmark: any; index: number }) {
  const card: KanbanCard = {
    ...
    url: bookmark.url,
    favicon: `https://www.google.com/s2/favicons?domain=${new URL(bookmark.url || 'https://example.com').hostname}&sz=64`,
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes}>
      <Card className="mb-3 hover:shadow-md transition-all duration-200 cursor-pointer group border-l-4 relative" 
            style={{ borderLeftColor: card.isFavorite ? '#f59e0b' : 'transparent' }}>
        {/* Drag Handle */}
        <div {...listeners} className="absolute top-2 right-2 p-1 opacity-0 group-hover:opacity-100 transition-opacity cursor-grab">
...
export const KanbanBoard2: React.FC<KanbanBoard2Props> = ({ bookmarks, onBookmarkClick, onSave }) => {
  ...
  // Helper function to group columns into rows of 4
  ...
  useEffect(() => {
    ...
    const bookmarkCards: KanbanCard[] = bookmarks.slice(0, 10).map((bookmark, index) => ({
      id: bookmark.id?.toString() || `bookmark-${index}`,
      title: bookmark.title || 'Untitled',
      description: bookmark.description || bookmark.ai_summary || '',
      url: bookmark.url,
  ...
  const handleAddFromBookmark = (bookmark: any) => {
    if (!selectedColumnId) {
      console.error('No column selected for adding bookmark');
      return;
    }
    
    console.log('Adding bookmark to column:', selectedColumnId, bookmark);
    ...
    setColumns(prev => {
      ...
      console.log('Updated columns after adding bookmark:', updated);
      onSave?.(updated); // Call onSave with updated columns
      return updated;
    });
                ...
                {bookmarks.map((bookmark, index) => (
                  ...
                  >
                    <div className="flex items-center space-x-3 flex-1 min-w-0">
                      <img 
                        src={`https://www.google.com/s2/favicons?domain=${new URL(bookmark.url || 'https://example.com').hostname}&sz=64`}
                        alt=""
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Kanban Board - Organized in rows of 4 columns */}
      <DndContext
        sensors={sensors}
              ...
              {!collapsedRows.has(rowIndex) && (
                <div className="w-full overflow-x-auto">
                  <div
                    ...
                    style={{ minWidth: `${columnRow.length * 320 + (columnRow.length - 1) * 24 + 8}px` }}
                  >
