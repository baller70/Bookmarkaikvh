...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: rpcData, error: rpcError } = await supabase.rpc('exec', { sql: userBookmarksMigrationSQL });

    if (rpcError) {
      console.log('RPC exec failed, trying alternative approach:', rpcError.message);

      // Try using sql function if it exists
      const { data: sqlData, error: sqlError } = await supabase.rpc('sql', { query: userBookmarksMigrationSQL });

      if (sqlError) {
        console.log('SQL RPC failed, trying direct query:', sqlError.message);

        // Try using query function
        const { data: queryData, error: queryError } = await supabase.rpc('query', { sql: userBookmarksMigrationSQL });
          ...
          try {
            const { data: rawData, error: rawError } = await supabase
              ...
              .insert({
                ...
                statements: [userBookmarksMigrationSQL]
              });
