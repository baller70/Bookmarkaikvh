...
const ExtensionInstallCard: React.FC = () => {
  ...
  const openGuide = () => {
    window.open('/docs/BROWSER_EXTENSION_ENHANCEMENT', '_blank', 'noopener,noreferrer')
  }
    ...
    <Card className="border-blue-200 bg-blue-50/30">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center space-x-3">
          <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={openStore}>
    ...
    <Card className="text-center py-12">
      <CardContent>
        <div className="max-w-md mx-auto">
          ...
          <Button variant="outline" className="text-sm" onClick={openGuide}>
            <Info className="h-4 w-4 mr-2" />
...
const BrowserLauncherProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (user) {
        try {
          const remote = await getAISetting<BrowserLauncherPrefs>(user.id, 'browser_launcher')
