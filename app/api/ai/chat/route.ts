...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      console.warn('⚠️ OpenAI API key not configured – returning safe fallback response')
      let fallbackBody: any = null
      try {
        fallbackBody = await request.json()
      } catch {}
      const msg = fallbackBody?.message || ''
      return NextResponse.json({
        success: true,
        response: msg
          ? `Oracle is temporarily unavailable. Echoing your message: "${msg}"\n\nThis is a fallback response (no API key configured).`
          : 'Oracle is temporarily unavailable. This is a fallback response (no API key configured).',
        usage: null,
